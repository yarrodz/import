...
class iFrameTransferProcess extends BaseVertexModel {
  ...
  async qq(query, sortings, first_only = false, author, label) {
    ...
    if (first_only) {
      ...
      traversal = traversal
        .project('transfer', 'import', 'unit')
        .by(__.valueMap(true))
        .by(__.in_('inImport').valueMap(true).fold())
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        let transfer = {};
        for (let [key, value] of result.get('transfer')) {
          transfer[key] = Array.isArray(value) ? value[0] : value;
        }
        transfer.label = 'iFrameTransfer';
        let imports = result.get('import');
        //   console.log('imports: ', imports);
        for (let j = 0; j < imports.length; j++) {
          let impt = imports[j];
          let newImpt = {};
          for (let [key, value] of impt) {
            newImpt[key] = Array.isArray(value) ? value[0] : value;
          }
          newImpt.label = 'iFrameProcess';
          imports[j] = newImpt;
        }
        ...
        results[i] = { ...transfer, import: imports[0], unit: units[0] };
      }
...
module.exports = { iFrameTransferProcess };
